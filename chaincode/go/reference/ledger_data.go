...
type LedgerData interface {
	...
	ExistsIn(stub shim.ChaincodeStubInterface) bool

	LoadFrom(stub shim.ChaincodeStubInterface) error

	UpdateOrInsertIn(stub shim.ChaincodeStubInterface) error
}
...
func Query(stub shim.ChaincodeStubInterface, index string, partialKey []string,
	...
	for it.HasNext() {
		...
		// replace with a factory method call
		entry := createEntry()
